"""Tests for eval_aggregator — manifest.json scanning and aggregation."""
...
def _write_manifest(task_dir: Path, data: dict) -> None:
...
def test_aggregate_orders_by_created_at_desc(tmp_path):
    tasks = tmp_path / "tasks"
    _write_manifest(tasks / "0001_a", _manifest("0001_a", created_at="2026-05-01T10:00:00+00:00"))
    _write_manifest(tasks / "0002_b", _manifest("0002_b", created_at="2026-05-08T10:00:00+00:00"))
    _write_manifest(tasks / "0003_c", _manifest("0003_c", created_at="2026-05-05T10:00:00+00:00"))
...
def test_aggregate_respects_last_n_cap(tmp_path):
    ...
    for i in range(5):
        _write_manifest(
            tasks / f"000{i}_x",
            _manifest(f"000{i}_x", created_at=f"2026-05-0{i + 1}T10:00:00+00:00"),
        )
    agg = aggregate_recent_runs(tasks, last_n=3)
    assert agg["total_runs"] == 3


def test_aggregate_skips_non_manifest_dirs(tmp_path):
    tasks = tmp_path / "tasks"
    tasks.mkdir()
    (tasks / "not_a_task").mkdir()  # no manifest.json
    _write_manifest(tasks / "0001_real", _manifest("0001_real"))
    agg = aggregate_recent_runs(tasks)
    assert agg["total_runs"] == 1


def test_aggregate_skips_invalid_json(tmp_path):
    tasks = tmp_path / "tasks"
    tasks.mkdir()
    bad = tasks / "0001_bad"
    bad.mkdir()
    (bad / "manifest.json").write_text("not valid json {{{")
    _write_manifest(tasks / "0002_good", _manifest("0002_good"))
    agg = aggregate_recent_runs(tasks)
    assert agg["total_runs"] == 1
...
def test_aggregate_status_and_mode_breakdown(tmp_path):
    tasks = tmp_path / "tasks"
    _write_manifest(tasks / "0001", _manifest("0001", status="completed", mode="feature"))
    _write_manifest(tasks / "0002", _manifest("0002", status="completed", mode="from_problem"))
    _write_manifest(tasks / "0003", _manifest("0003", status="escalated", mode="feature"))
    agg = aggregate_recent_runs(tasks)
...
def test_aggregate_loop_counts_summed(tmp_path):
    tasks = tmp_path / "tasks"
    _write_manifest(tasks / "0001", _manifest("0001", redesign_loops=1, final_loops=2))
    _write_manifest(tasks / "0002", _manifest("0002", redesign_loops=0, final_loops=3))
    agg = aggregate_recent_runs(tasks)
...
def test_aggregate_cost_trend_oldest_to_newest(tmp_path):
    tasks = tmp_path / "tasks"
    _write_manifest(
        tasks / "0001", _manifest("0001", cost=10.0, created_at="2026-05-01T10:00:00+00:00")
    )
    _write_manifest(
        tasks / "0002", _manifest("0002", cost=20.0, created_at="2026-05-05T10:00:00+00:00")
    )
    _write_manifest(
        tasks / "0003", _manifest("0003", cost=15.0, created_at="2026-05-08T10:00:00+00:00")
    )
    agg = aggregate_recent_runs(tasks)
...
def test_aggregate_stages_means_and_max():
    runs = [
        {
            "stages": {
                "rfc": {"attempts_count": 2, "duration_s": 100, "cost_usd": 2.0},
                "critique": {"attempts_count": 1, "duration_s": 50, "cost_usd": 1.0},
            }
        },
        {
            "stages": {
                "rfc": {"attempts_count": 4, "duration_s": 200, "cost_usd": 4.0},
                "critique": {"attempts_count": 1, "duration_s": 60, "cost_usd": 1.5},
            }
        },
    ]
